...
def apply_professional_css():
    """Apply enhanced professional CSS framework"""
    st.markdown("""
    <style>
        /* ===========================================
           SMART MONEY PROFESSIONAL UI FRAMEWORK
           =========================================== */
        ...
        :root {
            ...
            --border-color: #e8eaf6;
            --shadow-soft: 0 2px 12px rgba(26, 35, 126, 0.08);
            --shadow-medium: 0 4px 20px rgba(26, 35, 126, 0.12);
            --shadow-strong: 0 8px 32px rgba(26, 35, 126, 0.16);
...
# =================== ENHANCED PROFESSIONAL UI FUNCTIONS ===================

def create_large_professional_chart(fig_object):
    """Simple wrapper to make charts larger and more professional"""

    st.markdown('<div class="chart-container-large">', unsafe_allow_html=True)

    # Enhanced configuration for professional appearance
    config = {
        'displayModeBar': True,
        'displaylogo': False,
        ...
        'toImageButtonOptions': {
            'format': 'png',
            'filename': 'smartstock_professional_chart',
            'height': 800,
            'width': 1600,
            'scale': 2
        }
...
def add_wyckoff_stages_simple(fig, data):
    """Add simple Wyckoff stage annotations to chart"""
    ...
    # Find key points every 50 bars to avoid cluttering
    for i in range(50, len(data), 50):
        ...
        try:
            ...
            if current_volume > avg_volume * 1.5:  # High volume periods
            ...
            fig.add_annotation(
                ...
                bordercolor="white",
                borderwidth=1,
                font=dict(color="white", size=10, family="Inter", weight="bold"),
                opacity=0.9,
                ax=0,
                ay=-30
            )
...
# ADD THIS CALL at the beginning of main() function (after set_page_config)
...
def main():
    ...
    def create_professional_header():
        """Create world-class professional header"""
        current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

        header_html = f"""
        <div class="smart-money-header fade-in">
            <div style="display: flex; justify-content: space-between; align-items: center; position: relative; z-index: 1;">
                <div style="flex: 1;">
                    <h1 style="margin: 0; font-size: 2.8rem; font-weight: 700;">
                        📈 SmartStock AI Professional v2.0
                    </h1>
                    <p style="margin: 0.5rem 0 0; font-size: 1.2rem; opacity: 0.95;">
                        Institutional-Grade Trading Analysis Platform
                    </p>
                    <div style="margin-top: 1rem; display: flex; gap: 1rem; flex-wrap: wrap;">
                        <span style="background: rgba(0, 200, 83, 0.9); color: white; padding: 0.5rem 1rem; 
                                    border-radius: 20px; font-size: 0.875rem; font-weight: 600;">
                            ✅ LIVE SESSION
                        </span>
                        <span style="background: rgba(26, 35, 126, 0.9); color: white; padding: 0.5rem 1rem; 
                                    border-radius: 20px; font-size: 0.875rem; font-weight: 600;">
                            👤 User: wahabsust
                        </span>
                        <span style="background: rgba(26, 35, 126, 0.9); color: white; padding: 0.5rem 1rem; 
                ...
                <div style="text-align: right; opacity: 0.8;">
                    <div style="font-size: 0.9rem; margin-bottom: 0.5rem;">Session ID</div>
                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.8rem;">
                        {str(id(st.session_state))[-8:]}
                    </div>
                </div>
            </div>
        </div>
    ...
    try:
        # Initialize session state if needed (from first function)
        if 'ai_agent' not in st.session_state:
            st.session_state.ai_agent = EnhancedStockMarketAIAgent()
            st.session_state.analysis_complete = False
            st.session_state.data_loaded = False
...
def display_comprehensive_model_breakdown():
    ...
    create_large_professional_chart(lambda: fig)
...
def create_comprehensive_professional_dashboard(data, theme):
    ...
    fig.update_layout(
        title=dict(
            text="📊 SmartStock AI Professional - Institutional Trading Dashboard",
            font=dict(size=26, family="Inter", color='#1a237e', weight=700),
            x=0.5
        ),
        height=900,  # Increased for better visibility
        showlegend=True,
        template=template,
        font=dict(family="Inter", size=12),
        plot_bgcolor='rgba(248, 249, 255, 0.3)',
        paper_bgcolor='white',
        ...
        annotations=[
            dict(
                text=f"Generated by SmartStock AI v2.0 Professional | User: wahabsust | 2025-06-16 19:48:45 UTC",
                xref="paper", yref="paper",
                x=0.5, y=-0.08,
                showarrow=False,
                font=dict(size=10, color='#1a237e', family="Inter")
            )
        ]
    )
